...
public class UserToBanReply implements Reply<Message> {
    ...
    public BotApiMethod<?> getReply(Message message) {
        ...
        if (user.isPresent()) {
            ...
            userRepository.setStatus(telegramUser.getId(), TelegramUserStatus.BANNED);
            return SendMessage.builder()
                .chatId(message.getChatId().toString())
                .text(BotMessage.USER_BANNED.getMessage().formatted(telegramUser.getUserName()))
                .build();
        } else {
            return SendMessage.builder()
                .chatId(message.getChatId().toString())
