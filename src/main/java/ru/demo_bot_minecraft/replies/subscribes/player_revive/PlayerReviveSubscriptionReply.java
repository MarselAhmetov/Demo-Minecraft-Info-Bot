...
public class PlayerReviveSubscriptionReply implements Reply<Message> {
    ...
    public boolean predicate(Message message) {
        return messageEquals(message, RequestMessagesEnum.PLAYERS_REVIVE_SUBSCRIPTION);
    }

    public BotApiMethod<?> getReply(Message message) {
        subscriptionRepository.save(Subscription.builder()
            .telegramUser(TelegramUser.builder().id(message.getFrom().getId()).build())
            .type(SubscriptionType.PLAYERS_REVIVE)
            .build());
        SendMessage sendMessage = new SendMessage(message.getChatId().toString(), BotMessageEnum.PLAYERS_REVIVE_SUBSCRIBED.getMessage());
